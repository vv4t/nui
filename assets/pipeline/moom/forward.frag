...
void main()
{
  ...
  light += 0.1;
  light += calc_ssr_no_env(d_pos, d_albedo, vs_pos, vs_frag_pos.xyz, screen_uv, normal) * get_specular() * 1.0;
  
  frag_color = get_diffuse() * vec4(vec3(1.0), get_alpha()) * vec4(light, 1.0);
}
